...
def parse_experiment_data_par(file_path):
    try:
        ...
        (E_a, E_p, E_t, adjc, G, avgZArr, clustCoef, degreeDist, eigVal, eigVec,
         env_noise_std, mean_agent_measurement_noise, n_agents, range_agent_measurement_noise,
         scalar_param1, steps, weight_own_belief, z_gt, zArr,
         pArr, correlation_network_information) = extract_relevant_data_from_files(
            file_path)
...
def extract_relevant_data_from_files(file_path):
    ...
    return E_a, E_p, E_t, adjc, G, avgZArr, clustCoef, degreeDist, eigVal, eigVec, env_noise_std, mean_agent_measurement_noise, n_agents, range_agent_measurement_noise, scalar_param1, steps, weight_own_belief, z_gt, zArr, pArr, correlation_network_information
...
def parse_experiments_check(base_file_path):
    pathes = glob.glob(os.path.join(base_file_path, "*"))
    print("check pathes: " + pathes[0])

    dataframes = parse_experiment_data_par(pathes[0])
    print(dataframes)
    print(dataframes.Netw_eigVals_vs_time.shape)


if __name__ == "__main__":
    # # (anti-)correlation of centrality and certainty
    experiment_name_list = ["N100_2023-09-29-23-36-25_correlation_search_Naive_centralized_random_fixed_mdeg/results",
                            "N100_2023-09-30-01-22-49_correlation_search_NaiveLO_centralized_random_fixed_mdeg/results"]
